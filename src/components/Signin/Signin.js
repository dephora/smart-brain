...
class Signin extends React.Component {
  ...
  saveAuthTokenInSession = (token) => {
    window.sessionStorage.setItem('token', token) // prefer this
    // window.localStorage.setItem('token', token) // persists longer
  }
  onSubmitSignIn = () => {
        ...
        if (data.userId && data.success === 'true') {
          ...
          fetch(`http://localhost:3000/profile/${data.userId}`, {
            ...
            headers: {
              ...
              'Authorization': data.token  // would usually do Bearer
            }
          })
            .then(resp => resp.json())
            .then(user => {
